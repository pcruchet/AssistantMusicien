...
void LecteurAudio::LancerChanson(QStringList &selection, bool exterieur=false)
{
    QList<QMediaContent> musiques;

    if(nouvelleListe || exterieur)
    {
        playlist->clear();
        ui->listWidgetListeLecture->clear();
    }

    for(const QString& chanson : selection)
...
{
    QSize tailleLecteurAudio = event->size() ;
...
{
    if(_status != QMediaPlayer::LoadingMedia)
...
void LecteurAudio::on_pushButtonAjouter_clicked()
{
    ...
    if( fileDialog.exec())
    {
        QStringList listChansons = fileDialog.selectedFiles();
        LancerChanson(listChansons);
//        QList<QMediaContent> musiques;
